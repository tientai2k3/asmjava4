...
<body>
<div class="container">
    <form action="/Buoi2_war_exploded/gio-hang-chi-tiet/update?idGioHang=${ghct.idGioHang.id}&idChiTietSP=${ghct.idChiTietSP.id}" method="POST">
        <div class="row">
            <div class="col-md-6">
                ...
                <select class="form-select" aria-label="Default select example" name="gioHang">
                    <c:forEach items="${dsGioHang}" var="gh">
                        <option value="${gh.id}" ${gh.id == ghct.idGioHang.id ?"Selected":""}>${gh.ma}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select class="form-select" aria-label="Default select example" name="chiTietSP">
                     <c:forEach items="${dsCTSP}" var="ch">
                         <option value="${ch.id}" ${ch.id == ghct.idChiTietSP.id ?"Selected":""}>${ch.idSP.ma}</option>
                     </c:forEach>
                </select>
            </div>
        </div>
