...
<hr>
<br>
<a href="/Buoi2_war_exploded/shop/createGioHang" class="btn btn-outline-danger">Thêm mới</a>
<table class="table">
    <thead>
    <tr>
    ...
    <c:if test="${f:length(ds) !=0}">
        <c:forEach items="${ds}" var="gh">
            <tr>
                ...
                <td>${gh.diaChi}</td>
                <td>${gh.sdt}</td>
                <td>${gh.tinhTrang == 0? "Chưa xác nhận":"Đã xác nhận"}</td>
                <td><a href="indexGHCT?id=${gh.id}&ma=${gh.ma}" class="btn btn-success">Thêm sản phẩm vào giỏ hàng</a></td>
