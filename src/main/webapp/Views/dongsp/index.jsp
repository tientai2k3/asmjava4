    ...
    <table class="table">
        ...
        <c:if test="${f:length(ds)!=0}">
            <c:forEach items="${ds}" var="dsp">
                <tr>
                    <th scope="row">${dsp.ma}</th>
                    <td>${dsp.ten}</td>
                    <td><a href="edit?ma=${dsp.ma}" class="btn btn-success">Sửa</a></td>
                    <td><a href="delete?ma=${dsp.ma}"  class="btn btn-danger">Xóa</a></td>
                </tr>
            </c:forEach>
        </c:if>
