    ...
    <c:if test="${f:length(ds) !=0}">
        <c:forEach items="${ds}" var="hd">
            <tr>
                ...
                <td>${hd.idNV.ma}</td>
                <td>${hd.ngayTao}</td>
                <td>${hd.ngayThanhToan == null ?"Chưa thanh toán":hd.ngayThanhToan}</td>
                <td>${hd.ngayShip}</td>
                <td>${hd.ngayNhan}</td>
                ...
                <td>${hd.diaChi}</td>
                <td>${hd.sdt}</td>
                <td><c:if test="${hd.tinhTrang ==1}"><a href="thanh-toan?idHD=${hd.id}" class="btn btn-success">Xác nhận</a></c:if></td>
<%--                <td>  <a href="thanh-toan?idHD=${hd.id}" class="btn btn-success">Xác nhận</a></td>--%>
            </tr>
        </c:forEach>
    </c:if>
    </tbody>
